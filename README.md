...
现有基础课程以 **Linux kernel 5.10** 为事实基线：已经收章的章节按各自验收记录完成核验，仍在编写或待验收的章节以对应领域 README、completion review 和实验记录中的实际状态为准。新启用的 [`network/`](network/) 专题按其目录规则使用 **upstream Linux v6.12**。不同版本的实现不能混写成同一条调用路径，也不能通过机械替换版本号宣称已经完成迁移。
