...
- `docs/` 只保存具体章节，不再放第二份课程总纲；
- `labs/` 只保存实验、脚本和实验说明；
- `source-paths/` 只保存源码入口、调用路径和关键数据结构；
- 根目录 `README.md` 只说明全局范围、目录关系和推荐学习顺序。

这样可以从目录名确定知识领域，从该目录的 `README.md` 找到唯一的课程大纲，避免同一领域出现多份相互重叠的规划。
...
```text
README.md               本领域唯一的课程大纲
docs/                   分章节教程
labs/                   实验、脚本和调试记录
source-paths/           Linux 5.10 源码位置、调用路径和关键数据结构
```

Git 不保存空目录。因此，在具体章节尚未开始编写时，某个领域目录中可能只有 `README.md`。
