...
实验已建立 7 组 source-contract checker，以及 1 个完整正例 + 21 个独立负例的 22-case fixture coverage。此前 9-case fixture revision 的 PASS 只属于历史工具证据，不能继承给当前 exact revision。当前 B06 还缺两个收章硬门槛：当前 exact 22-case suite 必须实际得到 `22 tests / OK / exit code 0`；同一个 checker 必须对 upstream Linux v5.10 commit `2c85ebc57b3e1817b6ce1a6b703928e113a90442` 的真实源码得到 7/7 L1 PASS。人工源码复核与自动 checker PASS 是两类不同证据，不互相替代。

在这两个自动验收门槛完成前，B06 不标记为已完成，也不进入 B07。仓库已经提供仅手工触发、仅使用 `[self-hosted, linux, x64, kernel-course]` runner 的验收 workflow；没有匹配 self-hosted runner 时属于执行环境 blocker，不切换到可能产生额外费用的 GitHub-hosted runner。
...
B06 的课程内容边界仍到“下一映像已经准备并由旧内核持有，未来 execute/crash event 才消费它”的生命周期模型结束；只有上述验收闭环后，才从 B07 开始展开 segment/page-list 与 file loader 的具体装载机制。
