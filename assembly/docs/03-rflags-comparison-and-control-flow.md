...
一种设计是把比较结果写入普通寄存器，再由跳转指令读取该寄存器。x86 采用了另一种历史悠久的设计：
...
`inc` 和 `dec` 会更新多个标志位，但不会修改 `CF`。这是历史兼容语义。
...
编译器经常选择 `test reg,reg`，因为它表达清晰且编码通常紧凑。
...
这类代码在 Linux 内核的状态位、特性位和协议标志处理中极其常见。
...
位模式 `0x80` 按有符号解释为 `-128`。两个正数相加得到负数，说明有符号结果不可表示：
...
为什么不能简单用 `SF`？因为发生有符号溢出时，结果符号位可能与真实数学大小相反。`SF XOR OF` 才能校正这一情况。
...
注意 `setcc` 只写 8 位。若函数需要返回规范的 `int`，常见写法是先清零整个返回寄存器：
...
`cmovcc` 不改变控制流，可以避免某些短分支的预测失败，但并不总是更快：

- 两边表达式仍可能需要提前计算；
- 长路径或包含潜在故障的访存不能盲目改成条件移动；
- 可预测分支通常成本很低；
- 真实选择取决于编译器成本模型和微架构。
...
从 `.Lloop` 尾部跳回较低地址形成回边。识别回边是恢复循环结构的关键。
...
预测正确时，分支开销可能很低；预测失败时，错误路径上的推测工作需要被丢弃，并重新从正确目标取指。
...
Linux 内核中的 `likely()` / `unlikely()` 主要向编译器提供概率信息，影响代码布局和优化决策，并不是简单生成一条“硬件预测提示指令”。
...
C 类型为 `unsigned long` 时，通常应关注 `ja/jb` 系列，而不是 `jg/jl`。

### 16.3 以为 `cmp` 会修改操作数

`cmp` 只更新标志位，源和目标寄存器保持不变。
...
### 16.6 把 `CF` 和 `OF` 当成同一个概念

`CF` 服务无符号解释，`OF` 服务有符号解释。

### 16.7 只看汇编助记词，不看操作数重排

编译器可把 `a > b` 改写为 `b < a`，因此 `seta`、`setb` 或交换操作数可能表达同一高级语义。
